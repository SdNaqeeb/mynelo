...
import {
  BookOpen, Layers, FileText, Target, Check, CheckCircle, Search,
  ChevronDown, ChevronRight, ArrowLeft, Copy, Printer, Trash2,
  Filter, X, Info, AlertCircle, GraduationCap, Sparkles,
  BarChart3, Hash, TrendingUp, ScrollText, Zap, ClipboardList
} from 'lucide-react';

// Configuration
const ngrokApi = axios.create({
  baseURL: 'https://qd6z27lel2.execute-api.ap-south-1.amazonaws.com/api',
  headers: {
...
// API Service
const api = {
  async getClasses() { return (await ngrokApi.get('/classes/')).data; },
  async getClassDetails(id) { return (await ngrokApi.get(`/classes/${id}/`)).data; },
  async getClassWeightage(id) { return (await ngrokApi.get(`/classes/${id}/weightage/`)).data; },
  async getChapters(classId) { return (await ngrokApi.get(`/chapters/?class_id=${classId}`)).data; },
  async getChapterQuestions(chapterId) { return (await ngrokApi.get(`/chapters/${chapterId}/`)).data; },
  async getDashboardStats() { return (await ngrokApi.get('/dashboard/')).data; },
};
  ...
  const DashboardView = () => (
    <div className="animate-in fade-in duration-300 space-y-6">
      ...
      <div
        ...
        style={{ background: 'linear-gradient(135deg, #0B1120 0%, #0d2137 50%, #0a2d4d 100%)' }}
      >
        {/* background mesh dots */}
        <div className="absolute inset-0 opacity-10" style={{
          backgroundImage: 'radial-gradient(circle, #00A0E3 1px, transparent 1px)',
          backgroundSize: '28px 28px'
        }} />
        <div className="absolute top-0 right-0 w-64 h-64 rounded-full opacity-10"
          style={{ background: 'radial-gradient(circle, #00A0E3, transparent)', transform: 'translate(30%, -30%)' }} />
        <div className="relative z-10 px-6 py-8 sm:px-10 sm:py-10 flex flex-col sm:flex-row items-start sm:items-center gap-6">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0"
            style={{ background: 'rgba(0,160,227,0.2)', border: '1px solid rgba(0,160,227,0.4)' }}>
            <ClipboardList size={32} className="text-[#00A0E3]" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full"
                style={{ background: 'rgba(0,160,227,0.15)', color: '#00A0E3', border: '1px solid rgba(0,160,227,0.3)' }}>
                AI-Powered
              </span>
            </div>
            ...
            <p className="text-sm sm:text-base text-slate-400 mt-1.5">
              Select questions chapter-wise, track weightage, and generate print-ready exam papers.
            </p>
          </div>
          <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
            <div className="text-center px-4 py-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.06)' }}>
              <span className="block text-xl font-bold text-white">{Object.keys(selectedQuestions).length}</span>
              <span className="text-xs text-slate-400">Selected</span>
            </div>
            <div className="text-center px-4 py-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.06)' }}>
              <span className="block text-xl font-bold text-white">{Object.values(selectedQuestions).reduce((s, q) => s + (q.marks || 0), 0)}</span>
      ...
      {dashboardStats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {[
            { icon: GraduationCap, label: 'Classes', value: dashboardStats.total_classes || 0, color: '#00A0E3', bg: 'rgba(0,160,227,0.08)', border: 'rgba(0,160,227,0.18)' },
            { icon: Layers, label: 'Units', value: dashboardStats.total_units || 0, color: '#6366f1', bg: 'rgba(99,102,241,0.08)', border: 'rgba(99,102,241,0.18)' },
            { icon: BookOpen, label: 'Chapters', value: dashboardStats.total_chapters || 0, color: '#f59e0b', bg: 'rgba(245,158,11,0.08)', border: 'rgba(245,158,11,0.18)' },
            { icon: Hash, label: 'Questions', value: dashboardStats.total_questions || 0, color: '#10b981', bg: 'rgba(16,185,129,0.08)', border: 'rgba(16,185,129,0.18)' },
          ].map(({ icon: Icon, label, value, color, bg, border }) => (
            <div key={label} className="bg-white rounded-xl p-4 sm:p-5 flex items-center gap-3 sm:gap-4"
              ...
              <div className="min-w-0">
                <span className="block text-xl sm:text-2xl font-extrabold text-[#0B1120] leading-tight">{value.toLocaleString()}</span>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden"
        ...
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h2 className="text-base sm:text-lg font-bold text-[#0B1120] flex items-center gap-2">
              <ScrollText size={18} className="text-[#00A0E3]" /> Select a Class
            </h2>
            <p className="text-xs text-gray-400 mt-0.5">Choose a class to start building your question paper</p>
          </div>
          <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-[#00A0E3]/10 text-[#00A0E3]">
        ...
        {classes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center px-6">
            ...
            <h3 className="font-semibold text-gray-600">No classes available</h3>
            <p className="text-sm text-gray-400 mt-1">Check your API connection</p>
          </div>
        ) : (
          <div className="p-5 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {classes.map((cls, idx) => {
              const grad = CLASS_GRADIENTS[idx % CLASS_GRADIENTS.length];
              return (
                <button
                  key={cls.id}
                  className="group relative text-left w-full rounded-xl border border-gray-100 overflow-hidden transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-[#00A0E3]/40"
                  ...
                  <div className="p-4 pt-5">
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5"
                        ...
                        <GraduationCap size={18} style={{ color: grad.from }} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-[#0B1120] text-sm leading-tight truncate group-hover:text-[#00A0E3] transition-colors">
                          {cls.display_name}
                        </h3>
                        <div className="flex items-center gap-3 mt-2 flex-wrap">
                          <span className="flex items-center gap-1 text-xs text-gray-500">
                            <Layers size={12} className="text-gray-400" />
                            <span>{cls.total_units || '—'} Units</span>
                          </span>
                          <span className="flex items-center gap-1 text-xs text-gray-500">
                            <FileText size={12} className="text-gray-400" />
                            <span>{cls.total_chapters || '—'} Chapters</span>
                          </span>
                          {cls.total_marks && (
                            <span className="flex items-center gap-1 text-xs text-gray-500">
                              <Target size={12} className="text-gray-400" />
                              <span>{cls.total_marks}M</span>
                            </span>
                          )}
                        </div>
                      </div>
                      <ChevronRight size={16} className="text-gray-300 group-hover:text-[#00A0E3] transition-colors mt-1 flex-shrink-0" />
                    </div>
                  </div>
                </button>
              );
      ...
      {/* Quick tip */}
      <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-[#00A0E3]/5 border border-[#00A0E3]/15">
        <Zap size={16} className="text-[#00A0E3] flex-shrink-0 mt-0.5" />
        <p className="text-xs text-slate-600">
          <span className="font-semibold text-[#00A0E3]">Tip:</span> Your question selections are saved automatically — you can leave and come back without losing progress.
        </p>
      </div>
    </div>
  );
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
        <div className="w-14 h-14 rounded-2xl bg-[#00A0E3]/10 flex items-center justify-center">
          <ClipboardList size={28} className="text-[#00A0E3] animate-pulse" />
        </div>
        <div className="text-center">
          <p className="text-sm font-semibold text-[#0B1120]">Loading Question Paper Generator</p>
          <p className="text-xs text-gray-400 mt-1">Fetching classes and stats…</p>
        </div>
        <div className="w-8 h-8 border-[3px] border-[#00A0E3] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
